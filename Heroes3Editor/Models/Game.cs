...
namespace Heroes3Editor.Models
{
    public class Game
    {
        ...
        public bool isHota { get; set; } = true;
    ...
    public class Hero
    {
        ...
        public Hero(string name, Game game, int bytePosition)
        {
            ...
            for (int i = 0; i < 29; ++i)
            {
                var skillSlotIndex = _game.Bytes[BytePosition + Constants.HeroOffsets["SkillSlots"] + i + 1];

                if (skillSlotIndex != 0 && skillSlotIndex < 8)
                {
        ...
        public void UpdateSkill(int slot, string skill)
        {
            ...
            if (slot < NumOfSkills)
            {
                ...
                _game.Bytes[BytePosition + Constants.HeroOffsets["SkillSlots"] + 1 + Constants.Skills[oldSkill]] = 0;
            }

            Skills[slot] = skill;
            SkillLevels[slot] = skillLevel;
            _game.Bytes[BytePosition + Constants.HeroOffsets["Skills"] + Constants.Skills[skill]] = skillLevel;
            _game.Bytes[BytePosition + Constants.HeroOffsets["SkillSlots"] + 1 + Constants.Skills[skill]] = (byte)(slot + 1);
